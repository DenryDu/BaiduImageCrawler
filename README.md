# BaiduImageCrawler
百度图片爬虫，基于python3
个人学习开发用，适用于计算机视觉项目准备数据集用
包含图片数量验证，以及爬取动态链接和静态链接两种方式
提高了爬虫的容错率、稳定性,每30个一批进行图片下载
爬取图片命名兼顾考虑了文件管理器排序规则，通过定长命名（例如：00001）这种以图片需求总数位数为长度、其余位填充零的方式实现图片的有序排列

### 使用方法
- 通过/acjson路径爬取动态网页 (Recommended, faster and more stable than the other)
``` python
